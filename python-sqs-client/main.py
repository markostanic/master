...
def generate_messages(queue: QueueInfo, number_of_messages:int = Query(alias='number-of-messages', default=0), message_length: int = 10):
  for _ in range(number_of_messages):
    sqs_client.send_message(QueueUrl=queue.url, MessageBody=get_random_string(message_length))
  return None


@app.post('/api/v1/queues/{queue_name}/messages/generate', status_code=status.HTTP_204_NO_CONTENT, response_class=Response)
def generate_messages(queue_name, number_of_messages: int = Query(alias='number-of-messages', default=0), message_length: int = 10):
  queue = get_gueue(queue_name)
  for _ in range(number_of_messages):
